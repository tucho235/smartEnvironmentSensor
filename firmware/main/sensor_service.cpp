...
void update_snapshot(const SensorSample *sample, esp_err_t last_error)
{
...
void sensor_task(void *)
{
    ...
    while (true) {
        ...
        if (err == ESP_OK) {
            ESP_LOGI(TAG, "%s sensor task started", kSelectedSensorName);
            break;
        }

        ESP_LOGW(TAG, "%s init failed: %s; retrying in %lu ms",
                 kSelectedSensorName, esp_err_to_name(err), app_config::kSensorRetryIntervalMs);
    ...
    while (true) {
        ...
        if (err == ESP_OK) {
...
#else
            ESP_LOGI(TAG, "SHT30 sample: temperature=%.2f C, humidity=%.2f %%",
            ...
            update_snapshot(nullptr, err);
            ESP_LOGW(TAG, "Failed to read %s sample: %s", kSelectedSensorName, esp_err_to_name(err));
        }
